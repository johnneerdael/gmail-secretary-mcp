...
class TestTools:
    ...
    async def test_process_email(self, tools, mock_client, mock_context):
        ...
        result = await process_email(
            "INBOX", 123, "move", target_folder="Archive", ctx=mock_context
        )
        ...
        result = await process_email("INBOX", 123, "read", ctx=mock_context)
        ...
        result = await process_email("INBOX", 123, "unread", ctx=mock_context)
        ...
        result = await process_email("INBOX", 123, "flag", ctx=mock_context)
        ...
        result = await process_email("INBOX", 123, "delete", ctx=mock_context)

        mock_client.delete_email.assert_called_once_with(123, "INBOX")
        assert "Email deleted" in result

        # Test invalid action
        result = await process_email("INBOX", 123, "invalid_action", ctx=mock_context)
